...
public class OldLEDStrip extends SubsystemBase {
    ...
    public void runRainbow() {
        ticksPerSecond = 50;
        for (int i = 0; i < Constants.LED_LENGTH; i++) {
            setHSVMirrored((i + counter) % Constants.LED_LENGTH, (int) (((double) i / Constants.LED_LENGTH) * 180), 255,
                    255);
        }
    }
    ...
    public void update() {
        ...
        // when in tele and stowed without note show... nothing? alliance color? rainbow? idk
        // when in tele and ramping show "graph" of shooter velocity, show green when at setpoint (hopefully it'll shoot immediately)
